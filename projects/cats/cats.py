...
def accuracy(typed, source):
    ...
    while i < typed_len and i < source_len:
        if typed_words[i] == source_words[i]:
            acc_count += 1
        i += 1
    return acc_count / typed_len * 100
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    time_in_minute = elapsed / 60
    word_per_minute = word_num / time_in_minute
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for item in word_list:
        diff = diff_function(typed_word, item, limit)
        if diff < min_diff:
            min_diff = diff
            min_diff_word = item
    if min_diff <= limit:
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == prompt[i]:
            ...
            break
    progress = right_count / len(prompt)
    dct = {"id": user_id, "progress": progress}
    upload(dct)
    return progress
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    for item in times_per_player:
        lst = [item[i+1]-item[i] for i in range(len(item)-1)]
        time_cost.append(lst)
    return match(words, time_cost)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    res = [[] for _ in range(len(all_times))]
    for word_indice in word_indices:
        min_indice = 0
        for player_indice in player_indices:
            if all_times[player_indice][word_indice] < all_times[min_indice][word_indice]:
                min_indice = player_indice
        res[min_indice].append(all_words[word_indice])
    return res
    # END PROBLEM 10
