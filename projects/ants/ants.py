...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while True:
            if place.is_hive :
                return None
            if self.lower_bound <= count <= self.upper_bound and len(place.bees) > 0:
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing but has a large health value!"""
    name = 'Wall'
    food_cost = 4
    implemented = True
    ...
    def action(self, gamestate):
        if self.turns_to_chew:
            ...
            if len(self.place.bees) > 0:
                chosed_bee = random_bee(self.place.bees)
                chosed_bee.reduce_health(chosed_bee.health)
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if not gamestate.have_a_queen:
            gamestate.have_a_queen = True
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if target is not None:
            target.slow = 5
            target_action = Bee.action

            def new_action(gamestates):
                if target.slow > 0:
                    target.slow -= 1
                    if not gamestates.time % 2 :
                        target_action(target, gamestates)
                else:
                    target_action(target, gamestates)
