...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 3
    if num_rolls == 0:
        res = tail_points(opponent_score)
    else:
...
def is_always_roll(strategy, goal=GOAL):
    ...
    """
    # BEGIN PROBLEM 7
    fisrt_num = strategy(0, 0)
    for score0 in range(goal):
        for score1 in range(goal):
            if fisrt_num != strategy(score0, score1):
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    max_num_rolls = 1
    for i in range(1, 11):
        num_roll_dice = roll_dice
        average_dice = make_averaged(num_roll_dice ,total_samples)
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    tail_score = tail_points(opponent_score)
    new_score = score + tail_score
    if perfect_square(new_score):  
        new_score = next_perfect_square(new_score)  
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if perfect_square(tail_score):  
        tail_score = next_perfect_square(tail_score)  
    tail_score_increase = tail_score - score

    # average score
    # average_dice = make_averaged(roll_dice)
    # average_dice_6 = average_dice(6)
    average_dice_6 = 8.729

    # compare
    if tail_score >= 100 or tail_score_increase > average_dice_6:
        return 0
    else:
